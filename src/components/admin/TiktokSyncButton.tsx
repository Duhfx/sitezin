...
import {
  sincronizarTiktok,
  salvarSincronizacaoTiktok,
  type SyncResult,
} from "@/app/admin/(protected)/perfil/actions";
import type { SyncStep, SyncStepStatus } from "@/lib/tiktok-sync";
...
export default function TiktokSyncButton() {
  ...
  async function handleSave() {
    ...
    const r = await salvarSincronizacaoTiktok(result.payload);
    setSaving(false);
        ...
        >
          ...
          >
            <div className="flex shrink-0 items-center justify-between border-b border-border px-6 py-4">
              <h3 className="text-base font-semibold text-foreground">Sincronização do TikTok</h3>
              {!loading && !saving && (
                <button
                  type="button"
                  onClick={close}
            ...
            {loading && (
              <div className="flex items-center gap-3 px-6 py-8 text-sm text-muted-foreground">
                ...
                Buscando dados na API do TikTok…
              </div>
            )}
